...
let rec exp e = match e with
  ...
  | AsExpr (_, e1, ann) -> 
    squish [exp e1; text " as "; text ann]
  | DropExpr (_, e1, ann) -> squish [exp e1; text " drop "; text ann]
  | CpAnnExpr (_, e1, e2) -> squish [text "cpAnn"; 
      parens (horz [exp e1; text ", "; exp e2])]
  | AssertExpr (_, e1, ann) -> squish [text "Assert"; 
      parens (horz [exp e1; text " : "; text ann])]
  | SecAnnExpr (_, ann) -> squish [text "SecAnn "; text ann]
  | ExtendsExpr (_, ann1, ann2) -> 
      squish [text "SecAnn "; text ann1; text " Extends "; text ann2]
