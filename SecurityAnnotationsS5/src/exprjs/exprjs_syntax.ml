...
type expr = 
  ...
  | AsExpr of Pos.t * expr * ann
  | DropExpr of Pos.t * expr * ann
  | CpAnnExpr of Pos.t * expr * expr
  | AssertExpr of Pos.t * expr * ann
  | SecAnnExpr of Pos.t * ann
  | ExtendsExpr of Pos.t * ann * ann
...
let pos_of_expr expr = match expr with
  ...
  | AsExpr (pos, _, _) -> pos
  | DropExpr (pos, _, _) -> pos
  | CpAnnExpr (pos, _, _) -> pos
  | AssertExpr (pos, _, _) -> pos
  | SecAnnExpr (pos, _) -> pos
  | ExtendsExpr (pos, _, _) -> pos
