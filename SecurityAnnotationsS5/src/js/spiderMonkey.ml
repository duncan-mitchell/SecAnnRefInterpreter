...
and expr (v : json_type) : expr = 
  ...
  match typ with
    ...
    | "Call" ->
      let callee = expr (get "callee" v) in
      let arguments = map expr (list (get "arguments" v)) in
      (match callee with
        (*Security Annotation Special Operations*)
        | Id (_, "SecurityAnnotation.as") ->
          if ((List.length arguments) != 2) 
            then failwith "As called with too many arguments"
            else As (p, List.nth arguments 0, List.nth arguments 1)
        | Id (_, "SecurityAnnotation.drop") ->
          if ((List.length arguments) != 2) 
            then failwith "As called with too many arguments"
            else Drop (p, List.nth arguments 0, List.nth arguments 1)
        | Id (_, "SecurityAnnotation.cpAnn") ->
          if ((List.length arguments) != 2) 
            then failwith "As called with too many arguments"
            else CpAnn (p, List.nth arguments 0, List.nth arguments 1)
        | Id (_, "SecurityAnnotation.assert") ->
          if ((List.length arguments) != 2) 
            then failwith "As called with too many arguments"
            else Assert (p, List.nth arguments 0, List.nth arguments 1)
        | Id (_, "SecurityAnnotation.declare") ->
          if ((List.length arguments) != 1) 
            then failwith "As called with too many arguments"
            else SecAnn (p, List.nth arguments 0)
        | Id (_, "SecurityAnnotation.extends") ->
          if ((List.length arguments) != 2) 
            then failwith "As called with too many arguments"
            else Extends (p, List.nth arguments 0, List.nth arguments 1)
        | _ -> Call (p, callee, arguments))
    | ("Member" | "MemberExpression") -> 
      if bool (get "computed" v) then
	Bracket (p, expr (get "object" v), expr (get "property" v))
      else
        let obj = expr (get "object" v) in
        let ident = identifier (get "property" v) in
        (match expr (get "object" v) with
        (** Covers the annotation options*)  
        | Id (_, "SecurityAnnotation") -> 
          (match ident with
            | "as" -> Id (p, "SecurityAnnotation.as")
            | "drop" -> Id (p, "SecurityAnnotation.drop")
            | "cpAnn" -> Id (p, "SecurityAnnotation.cpAnn")
            | "assert" -> Id (p, "SecurityAnnotation.assert")
            | "declare" -> Id (p, "SecurityAnnotation.declare")
            | "extends" -> Id (p, "SecurityAnnotation.extends")
            | _ -> failwith "Invalid SecurityAnnotation Operation"
          )
        | _ ->
          Dot (p, obj, ident)
        )
