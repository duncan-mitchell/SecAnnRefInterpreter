...
let rec jse_to_exprjs (e : J.expr) : E.expr =
  ...
  | J.Object (p, mem_lst) ->
    ...
    | J.As (p, e1, e2) -> 
      let exprjs_e2 = jse_to_exprjs e2 in
      (match exprjs_e2 with 
       | E.String (_, ann) -> E.AsExpr (p, jse_to_exprjs e1, ann)
       | _ -> failwith "Annotation Not a String"
     )
    | J.Drop (p, e1, e2) ->
      let exprjs_e2 = jse_to_exprjs e2 in
      (match exprjs_e2 with 
       | E.String (_, ann) -> E.DropExpr (p, jse_to_exprjs e1, ann)
       | _ -> failwith "Annotation Not a String"
     )
    | J.CpAnn (p, e1, e2) -> 
      E.CpAnnExpr (p, jse_to_exprjs e1, jse_to_exprjs e2)
    | J.Assert (p, e1, e2) -> 
      let exprjs_e2 = jse_to_exprjs e2 in
      (match exprjs_e2 with 
       | E.String (_, ann) -> E.AssertExpr (p, jse_to_exprjs e1, ann)
       | _ -> failwith "Annotation Not a String"
     )
    | J.SecAnn (p, e1) ->
      let exprjs_e1 = jse_to_exprjs e1 in
      (match exprjs_e1 with 
       | E.String (_, ann) -> E.SecAnnExpr (p, ann)
       | _ -> failwith "Annotation Not a String"
     )
    | J.Extends (p, e1, e2) ->
      let exprjs_e1 = jse_to_exprjs e1 in
      let exprjs_e2 = jse_to_exprjs e2 in
      (match exprjs_e1 with 
       | E.String (_, ann1) -> (match exprjs_e2 with
          | E.String (_, ann2) -> E.ExtendsExpr(p, ann1, ann2)
          | _ -> failwith "Annotation Not a String")
       | _ -> failwith "Annotation Not a String"
     )
