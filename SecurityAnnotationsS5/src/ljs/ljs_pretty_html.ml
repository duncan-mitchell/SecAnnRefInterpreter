...
let html_of_value value =
  ...
  | True _ -> html_of_bool true
  | False _ -> html_of_bool false
  | Num (n, _) -> html_of_float n
  | String (s, _) -> html_of_string s
...
let html_of_answer answer filter =
  ...
  | Ljs_eval.Answer (_, _, envs, store) ->
    let store = (Store.to_map (get_1 store), Store.to_map (get_2 store)) in
