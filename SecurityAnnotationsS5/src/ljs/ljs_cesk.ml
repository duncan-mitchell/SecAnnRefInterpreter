...
let rec eval_cesk desugar clo store kont i =
  ...
  begin
    ...
    | C.ExpClos (S.String (_, s, annotation), env), _ ->
      eval (C.ValClos (V.String (s, annotation), env)) store kont
    | C.ExpClos (S.Num (_, n, annotation), env), _ ->
      eval (C.ValClos (V.Num (n, annotation), env)) store kont
    | C.ExpClos (S.True (_, annotation), env), _ ->
      eval (C.ValClos (V.True annotation, env)) store kont
    | C.ExpClos (S.False (_, annotation), env), _ ->
      eval (C.ValClos (V.False annotation, env)) store kont
        ...
        let attrs' = match oattr, valu with
          ...
          | S.Proto, _ ->
            failwith ("[interp] Update proto failed: " ^
                      (V.pretty_value valu))
          | S.Extensible, V.True _ -> { attrs with V.extensible=true }
          | S.Extensible, V.False _ -> { attrs with V.extensible=false }
          | S.Extensible, _ ->
    ...
    | C.ValClos (obj_val, env), K.OwnFieldNames k ->
      ...
      | V.ObjLoc loc ->
        ...
        let add_name n x m =
          P.IdMap.add (string_of_int x) (V.Data ({ V.value = V.String (n, [Top]); V.writable = false; },
                                             false,
                                             false)) m in
        ...
        let final_props =
          P.IdMap.add "length" (V.Data ({ V.value = V.Num (d, [Top]); V.writable = false; },
                                    false,
                                    false)) props in
      ...
      | V.ObjLoc loc, V.String (s, _) ->
        ...
        | attrs, props ->
          ...
          | V.Accessor (_, _, true) ->
            ...
            eval (C.ValClos (V.True [Top], env)) store' k
          | _ -> eval (C.LobClos (V.Throw ([], V.String ("unconfigurable-delete", [Top]), store))) store k
           with Not_found -> eval (C.ValClos (V.False [Top], env)) store k))
    ...
    | C.ValClos (body_val, env), K.SetField4 (p, obj_val, field_val, valu, env', k) ->
      ...
      | (V.ObjLoc loc, V.String (s, _)) ->
        ...
        let prop = L.get_prop p store obj_val s in
        let unwritable = (V.Throw ([], V.String("unwritable-field", [Top]), store)) in
        (match prop with
    ...
    | C.ValClos (pred_val, env), K.If (env', than, elze, k) ->
      if (pred_val = V.True [Top]) (*TODO: Likely wrong; shouldn't constrain*)
...
let eval_expr expr desugar print_trace =
  continue_eval expr desugar print_trace P.IdMap.empty (Store.empty, Store.empty, [Top])
