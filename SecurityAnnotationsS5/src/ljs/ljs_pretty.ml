...
let rec exp_helper exprec e = match e with
  | SecAnn (_, ann) -> squish [text "SecAnn "; text (pretty_ann ann)]
  | Extends (_, a1, a2) -> squish [text "SecAnn "; text (pretty_ann a1); text " Extends "; text (pretty_ann a2)]
  | Null _ -> text "null"
  | Undefined _ -> text "undefined"
  | Num (_,n, _) -> text (string_of_float n)
  | String (_,s, _) -> text ("\"" ^ (String.escaped s) ^ "\"")
  | True _ -> text "true"
  | False _-> text "false"
  | Id (p, x) -> text x
  | Object (p, avs, props, _) -> begin
    match props with
  ...
  | Lambda (p, xs, e) ->
    vert [squish [text "func"; parens (horz (intersperse (text ",") (map text xs)))];
          braces (exprec e)]
  (*TODO: LambdaEnf will be printing wrong*)
  | LambdaEnf (p, x_ann_s, e) ->
    let xs = List.map (function (x, y) -> 
        squish [text x; text " : "; text (pretty_ann y)]) x_ann_s in
    vert [squish [text "func"; parens (horz (intersperse (text ",") xs))];
          braces (exprec e)]
  (*Adding Annotations*)        
  | As (p, e, ann) ->
    squish [exprec e; text (" as "); text (pretty_ann ann)]
  (*Dropping Annotations*)
  | Drop (p, e, ann) ->
    squish [exprec e; text (" drop "); text (pretty_ann ann)]
  | CpAnn (p, e1, e2) ->
    squish [text "cpAnn"; parens ( horz [exprec e1; text ","; exprec e2])]
  | Eval (p, s, obj) -> 
