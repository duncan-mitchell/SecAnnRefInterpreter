...
let accessible_primordials init_ans ses_ans =
  let Ljs_eval.Answer (_, _, _, (init_obj_store, init_var_store, _)) = init_ans in
  let Ljs_eval.Answer (_, _, _, (ses_obj_store, ses_var_store, _)) = ses_ans in
...
let ses_check init_ans ses_ans =
  let Ljs_eval.Answer (init_exps, init_value, init_envs,
                       (init_obj_store, init_var_store, _)) = init_ans in
  let Ljs_eval.Answer (ses_exps, ses_value, ses_envs,
                       (ses_obj_store, ses_var_store, _)) = ses_ans in
...
(*  let accessible_primordials =
  ...
  List.iter (print_cycle (ses_obj_store, ses_var_store, Hierarchy.empty)) proto_cycles;
